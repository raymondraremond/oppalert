...
export default function AICareerAssistant({ opportunityId, oppTitle }: AICareerAssistantProps) {
  ...
  return (
    <div className="bg-surface/30 border border-border rounded-[2.5rem] p-8 md:p-10 backdrop-blur-md relative overflow-hidden group">
      <div className="absolute top-0 right-0 w-64 h-64 bg-emerald/5 blur-[80px] -z-10 group-hover:scale-110 transition-transform duration-700" />
      
      <div className="flex items-center gap-4 mb-8">
        <div className="w-12 h-12 rounded-2xl bg-emerald/10 flex items-center justify-center text-emerald shadow-inner animate-pulse-soft">
          <Sparkles size={24} />
        </div>
        <div>
          <h2 className="font-syne text-2xl font-bold text-primary italic">AI Career Assistant</h2>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald">Premium Alpha Feature</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <button
          ...
          disabled={!!loading}
          className="flex items-center justify-center gap-3 p-5 rounded-[1.5rem] bg-surface2 border border-border hover:border-emerald/40 hover:bg-surface transition-all group/btn disabled:opacity-50"
        >
          {loading === 'resume' ? (
            <div className="w-5 h-5 border-2 border-emerald/30 border-t-emerald rounded-full animate-spin" />
          ) : (
            <FileText size={20} className="text-emerald group-hover/btn:scale-110 transition-transform" />
          )}
        ...
        >
          {loading === 'cover_letter' ? (
            ...
            <FileSearch size={20} className="text-emerald group-hover/btn:scale-110 transition-transform" />
          )}
          ...
          >
            <div className="absolute top-4 right-4 z-10">
              <button 
                ...
                className="p-2.5 bg-bg/80 backdrop-blur-md border border-border rounded-xl text-primary hover:text-emerald transition-colors shadow-lg"
              >
            ...
            <div className="bg-bg/50 border border-border rounded-[2rem] p-8 max-h-[400px] overflow-y-auto scrollbar-hide relative group/result">
              <div className="absolute inset-0 bg-emerald/5 opacity-0 group-hover/result:opacity-100 transition-opacity pointer-events-none" />
