        ...
        <div className="w-full max-w-[440px] bg-surface/30 border border-border/60 rounded-[3rem] p-8 md:p-12 backdrop-blur-xl relative overflow-hidden group">
            ...
            {(['login', 'register'] as const).map(t => (
              <button
                ...
                className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${
                  tab === t 
                  ? 'bg-surface text-amber border border-amber/20 shadow-lg' 
          ...
          <form onSubmit={tab === 'login' ? handleLogin : handleRegister} className="space-y-4">
            {tab === 'register' && (
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-amber transition-colors" size={18} />
                  <input
                    type="text"
                    ...
                    required
                    className="w-full pl-12 pr-4 py-4 bg-surface2 border border-border rounded-2xl text-primary text-sm focus:border-amber/50 outline-none transition-all placeholder:text-muted/30 font-medium"
                  />
                </div>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-amber transition-colors" size={18} />
                <input
                  type="email"
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-amber transition-colors" size={18} />
                <input
                  type="password"
                  placeholder="••••••••"
                  value={password}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-5 bg-amber text-black font-black uppercase text-xs tracking-[0.2em] rounded-2xl hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-amber/10 disabled:opacity-50 disabled:cursor-not-allowed group mt-8"
            >
              {loading
                ? 'Processing...' 
                : (
          ...
          <p className="text-center text-[10px] text-muted/50 mt-10 font-bold uppercase tracking-widest leading-relaxed">
            ...
            <Link href="/terms" className="text-muted hover:text-amber underline decoration-amber/30">Terms of Service</Link> & <Link href="/privacy" className="text-muted hover:text-amber underline decoration-amber/30">Privacy Policy</Link>
          </p>
        </div>
      </ScrollReveal>
    </main>
