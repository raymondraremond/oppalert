...
export async function GET(req: NextRequest) {
  try {
    ...
    const alertRes = await pool.query('SELECT * FROM alert_preferences WHERE user_id = $1', [decoded.id]);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    // Check plan for instant_alerts
    if (instant_alerts === true) {
      const userRes = await pool.query('SELECT status FROM users WHERE id = $1', [decoded.id]);
      if (userRes.rows[0]?.status !== 'premium') {
        return NextResponse.json({ error: 'Instant alerts are only available for Premium users' }, { status: 403 });
      }
    }

    const upsertQuery = `
      INSERT INTO alert_preferences (
        user_id, new_opportunity_email, deadline_reminders, weekly_digest, instant_alerts, categories, countries
