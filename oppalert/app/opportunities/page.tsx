  ...
  return (
    <main className="min-h-screen bg-bg">
      ...
      <section className="relative pt-32 pb-20 px-6 bg-bg overflow-hidden border-b border-border/50">
        ...
        <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[600px] h-[400px] bg-emerald/5 blur-[120px] rounded-full pointer-events-none" />
        
        <div className="container mx-auto max-w-5xl relative z-10">
          <div className="text-center mb-10">
            <h1 className="font-serif text-4xl md:text-6xl text-primary mb-4 tracking-tight">
              Discover your next <span className="text-emerald italic">opportunity</span>.
            </h1>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="relative group p-2 bg-surface/60 backdrop-blur-2xl border border-glass-border rounded-full shadow-premium focus-within:border-emerald/40 transition-all duration-500">
              <div className="flex items-center gap-4 px-6 relative z-10">
                <Search className="text-emerald/70 group-focus-within:text-emerald transition-colors" size={24} />
                <input 
                  type="text" 
                ...
                <button className="hidden md:flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-emerald to-emerald-light text-[#080A07] font-bold rounded-full hover:scale-[1.02] active:scale-[0.98] transition-all shadow-glow-emerald text-sm tracking-wide">
                  Search
                </button>
              </div>
            </div>
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-12 py-6">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-6 py-3 rounded-full text-xs font-bold whitespace-nowrap transition-all duration-300 ${
                  activeCat === cat.id 
                    ? 'bg-emerald text-[#080A07] shadow-glow-emerald' 
                    : 'bg-surface/50 text-muted border border-border hover:border-emerald/40 hover:text-primary'
                }`}
              >
                {cat.label}
              </button>
          ...
          <div className="flex items-center gap-4 w-full lg:w-auto">
            <div className="flex-1 lg:flex-none cursor-pointer flex items-center justify-between lg:justify-start gap-3 px-5 py-3 bg-surface/50 border border-border hover:border-border2 rounded-xl text-xs font-bold text-primary transition-colors">
              ...
              <span className="text-emerald">Recent</span>
            ...
            <div className="flex items-center gap-1 p-1 bg-surface/50 border border-border rounded-xl">
              <button className="p-2.5 bg-emerald text-[#080A07] rounded-lg shadow-sm"><LayoutGrid size={16} /></button>
        ...
        {!loading && (
          <div className="mb-10 flex items-center gap-4">
            <div className="text-sm font-semibold text-primary">
              Showing <span className="text-emerald">{opps.length}</span> verified listings
            </div>
        ...
        {!loading && opps.length === 0 && (
          <div className="text-center py-32 bg-surface/10 border border-dashed border-border rounded-[3rem] mx-auto max-w-2xl">
            <div className="w-20 h-20 rounded-full bg-surface border border-border flex items-center justify-center mx-auto mb-8 text-emerald/40">
              <Search size={32} />
            </div>
            ...
            <button 
              ...
              className="px-8 py-3 bg-surface border border-border rounded-xl text-emerald font-black text-[10px] uppercase tracking-widest hover:bg-border transition-all"
            >
              Clear all filters
            </button>
          </div>
        ...
        {!loading && opps.length > 0 && (
          <div className="mt-20 flex justify-center gap-2">
            {[1, 2, 3].map(i => (
              <button key={i} className={`w-10 h-10 rounded-xl border border-border flex items-center justify-center font-bold text-xs transition-all ${i === 1 ? 'bg-emerald text-[#080A07] border-emerald' : 'bg-surface/30 text-muted hover:border-emerald/30'}`}>
                {i}
              </button>
            ))}
            <button className="px-4 h-10 rounded-xl border border-border bg-surface/30 text-muted font-bold text-xs hover:border-emerald/30 transition-all">Next</button>
          </div>
        )}
      </div>
    </main>
