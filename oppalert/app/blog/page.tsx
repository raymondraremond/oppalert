      ...
      <div className="container mx-auto max-w-7xl">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12 mb-20">
          <div className="max-w-3xl space-y-6">
            <Link href="/" className="group inline-flex items-center gap-3 text-amber text-[10px] font-black uppercase tracking-[0.3em] hover:translate-x-[-4px] transition-transform">
              <ArrowLeft size={16} /> Back to Hub
            </Link>
            <h4 className="text-[10px] font-black text-amber uppercase tracking-[0.4em] mb-4">Strategic Insights</h4>
            <h1 className="font-serif text-6xl md:text-8xl font-black tracking-tight leading-none">
              Career <span className="text-amber italic">Archives.</span>
            </h1>
          ...
          <div className="relative w-full lg:w-96 group">
             <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-amber transition-colors" size={20} />
             <input 
               type="text" 
               placeholder="Search the archives..."
               className="w-full bg-surface/30 border border-border/60 rounded-3xl pl-14 pr-6 py-5 text-sm font-bold focus:border-amber/40 outline-none transition-all shadow-xl shadow-black/20" 
             />
          </div>
        </div>
        ...
        <div className="relative group mb-24 cursor-pointer overflow-hidden rounded-[4rem] border border-border/50 shadow-premium aspect-[21/10] md:aspect-[21/8]">
           ...
           <div className="absolute bottom-0 left-0 p-8 md:p-16 max-w-3xl relative z-20">
              <div className="flex items-center gap-4 mb-6">
                 <span className="px-5 py-2 bg-amber text-black text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg shadow-amber/10">{BLOG_POSTS[0].category}</span>
                 <span className="text-xs text-muted font-bold flex items-center gap-2"><Clock size={14} className="text-amber" /> {BLOG_POSTS[0].readTime}</span>
              </div>
              ...
              <Link href={`/blog/${BLOG_POSTS[0].slug}`} className="group/btn relative px-10 py-5 bg-surface backdrop-blur-xl border border-border rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-amber hover:text-black transition-all flex items-center justify-center w-fit gap-3 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-16">
          {BLOG_POSTS.slice(1).map((post, i) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="group relative block stagger-children">
              <div className="aspect-[4/3] rounded-[3rem] overflow-hidden mb-8 border border-border/40 bg-surface/30 relative shadow-2xl transition-all duration-700 hover:border-amber/40">
                 <img 
                   src={post.image} 
                   alt={post.title} 
                 ...
                 <button 
                   ...
                   className="absolute top-6 right-6 p-4 rounded-2xl bg-bg/40 backdrop-blur-xl border border-white/5 text-muted hover:text-amber hover:bg-bg/80 transition-all z-20"
                 >
                    <Bookmark size={20} />
                 </button>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-2 text-[10px] font-black text-amber uppercase tracking-[0.3em] mb-2 opacity-80">
                   {post.category} <div className="w-1 h-1 rounded-full bg-border" /> {post.date}
                </div>
                <h3 className="font-serif text-3xl font-bold leading-[1.2] group-hover:text-amber transition-colors tracking-tight opacity-90">{post.title}</h3>
                ...
                <div className="pt-8 flex items-center justify-between border-t border-border/40">
                   <div className="flex items-center gap-3">
                     <div className="w-10 h-10 rounded-full bg-surface2 border border-border flex items-center justify-center text-[10px] font-black text-amber shadow-lg">{post.author?.[0]}</div>
                   ...
                   <div className="flex items-center gap-2 px-3 py-1.5 bg-surface/50 border border-border/50 rounded-full text-[9px] font-black uppercase tracking-[0.15em] text-muted">
                     <Clock size={12} className="text-amber" /> {post.readTime}
                   </div>
                </div>
              </div>
            </Link>
        ...
        <div className="mt-40 p-16 md:p-24 rounded-[5rem] bg-surface/30 backdrop-blur-3xl border border-border/50 relative overflow-hidden group">
           <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-amber/5 blur-[120px] rounded-full animate-float" />
           <div className="max-w-3xl mx-auto text-center relative z-10">
             <div className="w-16 h-16 bg-amber/10 border border-amber/20 rounded-2xl flex items-center justify-center text-amber mx-auto mb-10 shadow-glow-amber">
                <Zap size={32} />
             </div>
             ...
             <form className="max-w-md mx-auto relative group-focus-within:scale-[1.02] transition-transform">
                <input 
                  ...
                  className="w-full bg-bg border border-border/80 rounded-[2.5rem] px-8 py-5 text-sm outline-none focus:border-amber/50 transition-all placeholder:opacity-30"
                  required
                />
                <button className="absolute right-2 top-2 bottom-2 bg-amber text-black px-10 rounded-full font-black uppercase text-[10px] tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl shadow-amber/10">
                  Transmit
                </button>
             </form>
           </div>
