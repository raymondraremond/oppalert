...
export default function BlogPostDetail({ params }: { params: { slug: string } }) {
  ...
  if (!post) return (
    <div className="min-h-screen bg-bg flex flex-col items-center justify-center p-6 bg-glow-emerald">
      ...
      <Link href="/blog" className="px-8 py-4 bg-emerald text-black font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl shadow-emerald/10">Back to Archives</Link>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-bg text-primary pt-32 pb-40 px-6 relative overflow-hidden">
      {/* Background Glows */}
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-emerald/5 blur-[150px] rounded-full -z-10" />
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-emerald/5 blur-[120px] rounded-full -z-10" />

      <div className="container mx-auto max-w-4xl">
        <Link href="/blog" className="group inline-flex items-center gap-3 text-emerald text-[10px] font-black uppercase tracking-[0.3em] mb-12 hover:translate-x-[-4px] transition-transform">
          <ArrowLeft size={16} /> Back to Archives
        </Link>
        
        <header className="mb-16">
           <div className="flex flex-wrap items-center gap-4 mb-8">
              <span className="px-5 py-2 bg-emerald text-black text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg shadow-emerald/10">{post.category}</span>
              <div className="flex items-center gap-4 text-xs text-muted font-bold font-mono">
                 <span className="flex items-center gap-2"><Clock size={14} className="text-emerald" /> {post.readTime}</span>
                 <div className="w-1 h-1 rounded-full bg-border" />
                 <span className="flex items-center gap-2"><Calendar size={14} className="text-emerald" /> {post.date}</span>
              </div>
           </div>
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between py-10 border-y border-border/60 gap-8">
              <div className="flex items-center gap-5">
                 <div className="w-14 h-14 rounded-2xl bg-surface border border-border flex items-center justify-center font-black text-emerald text-xl shadow-inner relative overflow-hidden">
              ...
              <div className="flex gap-4">
                 <button 
                   ...
                   className="w-12 h-12 flex items-center justify-center bg-surface border border-border rounded-xl text-muted hover:text-emerald hover:border-emerald/40 transition-all active:scale-90"
                   title="Copy Link"
                 >
                    <Share2 size={18} />
                 </button>
        ...
        <article className="max-w-none">
           <div className="font-serif text-2xl md:text-3xl text-primary font-bold leading-relaxed mb-16 pl-10 border-l-4 border-emerald pt-2 pb-2 relative italic opacity-90">
             <div className="absolute -left-3 -top-1 font-serif text-6xl text-emerald/20 opacity-50">&quot;</div>
             {post.excerpt}
           </div>
        ...
        <footer className="mt-40 pt-20 border-t border-border/60">
           <div className="flex flex-col md:flex-row items-center justify-between gap-12 mb-24">
              <div>
                 <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald mb-4 text-center md:text-left">Social Transmission</h4>
              ...
              <div className="flex gap-6">
                 {shareLinks.map((link, i) => (
                    ...
                    >
                       <div className="w-14 h-14 rounded-2xl bg-surface border border-border flex items-center justify-center text-muted group-hover:bg-emerald group-hover:text-black group-hover:border-emerald transition-all duration-500 shadow-xl group-hover:shadow-emerald/20">
                          <link.icon size={20} />
                       </div>
           ...
           <div className="relative p-12 md:p-20 bg-surface/30 backdrop-blur-3xl border border-border/50 rounded-[4rem] overflow-hidden group">
              <div className="absolute top-0 right-0 w-64 h-64 bg-emerald/5 blur-[100px] rounded-full animate-pulse-soft" />
              <div className="relative z-10 text-center space-y-8">
                 <div className="w-14 h-14 bg-emerald/10 border border-emerald/20 rounded-2xl flex items-center justify-center text-emerald mx-auto shadow-glow-emerald">
                    <Zap size={24} />
                 </div>
                 ...
                 <Link href="/register" className="inline-flex relative px-12 py-5 bg-emerald text-black font-black uppercase text-xs tracking-[0.2em] rounded-2xl shadow-xl shadow-emerald/10 hover:scale-105 active:scale-95 transition-all">
